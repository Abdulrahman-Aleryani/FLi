...
from frappe.utils import cint, comma_and, cstr, get_datetime
from frappe.utils.file_manager import safe_b64decode
from fuzzywuzzy import fuzz

from lms.lms.doctype.course_lesson.course_lesson import save_progress
from lms.lms.quiz_availability import (
	ensure_quiz_can_start,
...
class LMSQuiz(Document):
	...
	def validate_availability_window(self):
		available_from = getattr(self, "available_from", None)
		available_until = getattr(self, "available_until", None)

		if available_from and available_until:
			start = get_datetime(available_from)
			end = get_datetime(available_until)
			if start >= end:
...
@frappe.whitelist()
def get_quiz_with_availability(name):
	quiz = frappe.get_doc("LMS Quiz", name)
	availability = get_quiz_availability(quiz)
	data = quiz.as_dict()
	data["availability"] = availability

	if not availability.can_view and not availability.is_staff:
