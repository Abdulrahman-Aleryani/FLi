...
def set_language_from_query():
	query_lang = frappe.form_dict.get("_lang")
	if not query_lang:
		return

	lang_code = get_lang_code(query_lang)
	if not lang_code:
		return

	frappe.local.lang = lang_code
	frappe.form_dict._lang = lang_code
	if hasattr(frappe.local, "cookie_manager"):
		frappe.local.cookie_manager.set_cookie("preferred_language", lang_code, deduplicate=True)
